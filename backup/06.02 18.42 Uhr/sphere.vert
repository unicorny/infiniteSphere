...
// make it spherical
	...
	vec4 nonScaledDir = gl_Vertex-vec4(SphericalCenter, 1.0);
	
	float l = length(dir);
	float lNS = length(nonScaledDir.xyz);
		
//calculate texture coordinates
	v_texCoord3D = normalize(vec4(gl_TextureMatrix[0] * normalize(nonScaledDir)).xyz);
	gl_TexCoord[0] = vec4(((-v_texCoord3D.xyz)*0.5+0.5), 1.0);//vec4(-transVertex.x*0.5+0.5, -transVertex.y*0.5+0.5, 0.0, 1.0);
